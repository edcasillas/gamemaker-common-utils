...
for (var _i = 0; _i < ds_list_size(subscribers); _i ++) {
    ...
    if (_instance == noone || !instance_exists(_instance)) {
        log_error("Invalid instance subscribed to gmcu_o_layered_gui_manager. Make sure to unsubscribe objects in the Clean Up event.");
    ...
    if (is_undefined(_instance.on_draw_gui)) {
        log_error("Subscriber of gmcu_o_layered_gui_manager does not have a function on_draw_gui.");
        ds_list_add(_invalid_indices, _i); // Mark index for removal
        continue;
    }
    ...
    try {
        ...
        log_exception(_exception, "on_draw_gui for instance: " + string(_instance));
        ds_list_add(_invalid_indices, _i); // Mark index for removal
    }
